<#
.SYNOPSIS
    Scans SharePoint Online sites and reports the file count and total size of all document libraries in each site collection.
...
.DESCRIPTION
    ...
    site URLs specified in an input file. For each site it enumerates all document libraries, then for each library recursively counts all
    files (not folders) and sums their sizes in bytes. Results are output per-library showing file count and total size in bytes, MB, and GB.
    The script logs its operations and outputs the results to a CSV file.
...
.OUTPUTS
    - Excel report: Single .xlsx workbook with two sheets
        * "Summary" sheet: one row per document library with file count and size totals
        * "Files" sheet: one row per file (written when $getFiles is "top" or "all")
      (path: $env:TEMP\AllDocLibraries_Report_[timestamp].xlsx)
    - Log file documenting the script's execution
      (path: $env:TEMP\AllDocLibraries_Report_[timestamp].txt)

.NOTES
    File Name      : Get-AllDocLibrariesReport.ps1
    Author         : Mike Lee
   Created         : 4/22/26
...
$requiredModules = @('Microsoft.Graph.Authentication', 'Microsoft.Graph.Sites', 'Microsoft.Graph.Files', 'ImportExcel')
...
$requiredCmdlets = @('Get-MgSite', 'Get-MgSiteList', 'Get-MgSiteListItem', 'Get-MgSiteDrive', 'Connect-MgGraph', 'Export-Excel')
...
$logFilePath = "$env:TEMP\AllDocLibraries_Report_$startime.txt"

# Set output file path (single Excel workbook)
$outputFilePath = "$env:TEMP\AllDocLibraries_Report_$startime.xlsx"
...
#region Function: Get-SafeSheetName
# Derives a safe Excel sheet name (max 31 chars) from a site URL.
# Uses the last non-empty URL path segment (e.g. "SPSite1" from ".../sites/SPSite1").
function Get-SafeSheetName {
    param ([string]$SiteURL)
    $uri = [System.Uri]$SiteURL
    $segments = $uri.AbsolutePath.Trim('/').Split('/') | Where-Object { $_ -ne '' }
    $name = if ($segments.Count -gt 0) { $segments[-1] } else { $uri.Host }
    # Remove characters invalid in Excel sheet names: \ / * ? : [ ]
    $name = $name -replace '[\\/*?:\[\]]', '_'
    if ($name.Length -gt 31) { $name = $name.Substring(0, 31) }
    return $name
}
#endregion Function: Get-SafeSheetName

#region Function: Get-TopFilesBySize
# Returns files per library sorted largest first. Pass $Top = [int]::MaxValue to retrieve all files.
function Get-TopFilesBySize {
    ...
    foreach ($f in $topFiles) {
        ...
        # parentReference.path is like "/drives/<driveId>/root:/Folder/SubFolder" — strip the drive prefix
        $rawPath = if ($f.parentReference -and $f.parentReference.path) { $f.parentReference.path } else { "" }
        $folderPath = if ($rawPath -match ':(.*)$') { $Matches[1] } else { "/" }
...
Write-Log "Output format: Excel (xlsx - ImportExcel)"
Write-Log "Targeting: all document libraries in each site collection"
...
foreach ($siteURL in $siteURLs) {
    ...
    try {
        ...
        # Include standard document libraries AND the PreservationHoldLibrary, which uses a
        # different template type ("preservationHoldLibrary" / 851) so must be matched by name.
        $docLibraries = $allLists | Where-Object {
            $_.List.Template -eq "documentLibrary" -or
            $_.DisplayName -eq "PreservationHoldLibrary" -or
            $_.DisplayName -eq "Preservation Hold Library" -or
            $_.Name -eq "PreservationHoldLibrary"
        }

        if (-not $docLibraries -or @($docLibraries).Count -eq 0) {
            Write-Log "No document libraries found on site $siteURL" "WARNING"
            $global:reportData.Add([PSCustomObject]@{
                    SiteURL        = $siteURL
                    LibraryName    = ""
                    LibraryFound   = $false
                    ...
                    TotalSizeGB    = 0
                    ProcessingTime = ((Get-Date) - $siteStartTime).ToString()
                    Notes          = "No document libraries found"
                })
            continue
        }

        Write-Host "Found $(@($docLibraries).Count) document library/libraries on $siteURL" -ForegroundColor Cyan
        Write-Log "Found $(@($docLibraries).Count) document library/libraries on $siteURL" "INFO"

        foreach ($library in $docLibraries) {
            $libraryStartTime = Get-Date
            $libraryName = $library.DisplayName
            Write-Log "Processing library: '$libraryName' on $siteURL" "INFO"

            # Get the drive ID for the library
            $driveId = Get-LibraryDriveId -siteId $siteId -list $library

            if ([string]::IsNullOrEmpty($driveId)) {
                Write-Log "Could not get drive ID for library '$libraryName' on $siteURL" "WARNING"
                $global:reportData.Add([PSCustomObject]@{
                        SiteURL        = $siteURL
                        LibraryName    = $libraryName
                        LibraryFound   = $true
                        ...
                        TotalSizeMB    = 0
                        TotalSizeGB    = 0
                        ProcessingTime = ((Get-Date) - $libraryStartTime).ToString()
            ...
            Write-Log "Enumerating all items in '$libraryName' via delta API..." "INFO"
            $allItems = [System.Collections.Generic.List[object]]::new()
            $deltaUri = "https://graph.microsoft.com/v1.0/drives/$driveId/root/delta?`$select=id,name,size,file,folder,deleted,parentReference"
            $pageNumber = 0
            ...
            try {
                do {
                    ...
                    if ($pageNumber % 5 -eq 0 -or -not $deltaUri) {
                        Write-Host "  ...retrieved $($allItems.Count) items so far (page $pageNumber) in '$libraryName'" -ForegroundColor DarkCyan
                    }
                } while ($deltaUri)

                Write-Log "Delta API enumeration complete: $($allItems.Count) total items across $pageNumber page(s) in '$libraryName'" "INFO"
            }
            catch {
                Write-Log "Delta API failed for '$libraryName': $($_.Exception.Message). Falling back to list items API..." "WARNING"

                # Fallback: use Get-MgSiteListItem with field expansion
                $allItems = [System.Collections.Generic.List[object]]::new()
                $rawListItems = Invoke-WithRetry -ScriptBlock {
                    return Get-MgSiteListItem -SiteId $siteId -ListId $library.Id -All -ExpandProperty "driveItem"
                }
                foreach ($li in $rawListItems) {
                    if ($li.DriveItem -and -not $li.DriveItem.Folder) {
                        $allItems.Add([PSCustomObject]@{
                                ...
                                folder          = $null
                                deleted         = $null
                                parentReference = $li.DriveItem.ParentReference
                            })
                    }
                }
                if ($debug) {
                    Write-Log "DEBUG - Fallback list items API returned $($allItems.Count) file items for '$libraryName'" "INFO"
                }
            }
            ...
            if ($getFiles -ne "none") {
                ...
                $siteFiles = Get-TopFilesBySize -Items $allItems -SiteURL $siteURL -LibraryName $libraryName -Top $topParam
                ...
                Write-Log "Collected $fileLabel file(s) from '$libraryName' for $siteURL" "INFO"
            }

            Write-Log "Library: '$libraryName' | Site: $siteURL | Files: $fileCount | Size: $totalMB MB ($totalGB GB)" "SUCCESS"
        ...
        Write-Log "Failed to process document libraries for site $siteURL. Error: $($_.Exception.Message)" "ERROR"
        $global:reportData.Add([PSCustomObject]@{
                SiteURL        = $siteURL
                LibraryName    = ""
                LibraryFound   = $false
...
# Group both report and file data by site URL
$summaryGroups = $global:reportData | Group-Object -Property SiteURL
$filesGroups = $global:topFilesData | Group-Object -Property SiteURL

# Track used sheet names across the whole workbook to avoid duplicates
$usedSheetNames = @{}

function Get-UniqueSheetName {
    param ([string]$Base)
    $name = $Base
    if ($usedSheetNames.ContainsKey($name)) {
        $usedSheetNames[$name]++
        $suffix = "_$($usedSheetNames[$name])"
        $name = $name.Substring(0, [Math]::Min($name.Length, 31 - $suffix.Length)) + $suffix
    }
    else {
        $usedSheetNames[$name] = 0
    }
    return $name
}

# Write one Summary sheet + one Files sheet per site collection
foreach ($group in $summaryGroups) {
    $siteBase = Get-SafeSheetName -SiteURL $group.Name

    # Summary sheet for this site
    $summarySheet = Get-UniqueSheetName -Base $siteBase
    try {
        $group.Group | Export-Excel -Path $outputFilePath -WorksheetName $summarySheet `
            -AutoSize -FreezeTopRow -BoldTopRow -TableStyle Medium9
        Write-Log "Summary sheet '$summarySheet' written for site: $($group.Name)" "SUCCESS"
    }
    catch {
        Write-Log "Failed to write summary sheet '$summarySheet' for $($group.Name): $($_.Exception.Message)" "ERROR"
    }

    # Files sheet for this site (only when $getFiles is not "none")
    if ($getFiles -ne "none") {
        $siteFiles = $filesGroups | Where-Object { $_.Name -eq $group.Name }
        if ($siteFiles -and $siteFiles.Group.Count -gt 0) {
            $filesBase = ($siteBase.Substring(0, [Math]::Min($siteBase.Length, 25))) + "_Files"
            $filesSheet = Get-UniqueSheetName -Base $filesBase
            try {
                $siteFiles.Group | Export-Excel -Path $outputFilePath -WorksheetName $filesSheet `
                    -AutoSize -FreezeTopRow -BoldTopRow -TableStyle Medium2
                Write-Log "Files sheet '$filesSheet' written for site: $($group.Name)" "SUCCESS"
            }
            catch {
                Write-Log "Failed to write files sheet '$filesSheet' for $($group.Name): $($_.Exception.Message)" "ERROR"
            }
        }
        else {
            Write-Log "No file data collected for site: $($group.Name)" "WARNING"
        }
    }
}
...
Write-Log "Scan completed. Report rows written: $($global:reportData.Count). Total processing time: $totalTime" "SUCCESS"
