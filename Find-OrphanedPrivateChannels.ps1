# ============================================================================
# Script: Find SharePoint Sites that are Teams Private Channels with Orphaned Groups
...
$outputFilePath = ".\PrivateChannels-Orphaned-Groups-Report-$(Get-Date -Format 'yyyyMMdd-HHmmss').csv"
...
$sites = $allSites | Where-Object { $_.GroupId -eq "00000000-0000-0000-0000-000000000000" -and $_.RelatedGroupId -ne "00000000-0000-0000-0000-000000000000" }

Write-Host "Found $($allSites.Count) total sites, $($sites.Count) are Private Channels. Processing..." -ForegroundColor Green
...
foreach ($site in $sites) {
    ...
    $siteReport = [PSCustomObject]@{
        ...
        URL                     = $site.Url
        Owner                   = $site.Owner
        IBMode                  = $site.InformationBarrierMode  # Not available via Get-PnPTenantSite
    ...
    if ($site.GroupId -eq "00000000-0000-0000-0000-000000000000" -and $site.RelatedGroupId -ne "00000000-0000-0000-0000-000000000000") {
        try {
            # Try to get the unified group
            $group = Get-UnifiedGroup -Identity $site.RelatedGroupId -ErrorAction SilentlyContinue
            
            if ($null -eq $group) {
                # Group doesn't exist - it's orphaned
                $siteReport.HasOrphanedGroup = "YES"
                Write-Host "  [ORPHANED] $($site.Title) - GroupID: $($site.RelatedGroupId)" -ForegroundColor Yellow
            }
            else {
                # Group exists
